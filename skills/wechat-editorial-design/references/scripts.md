...
```bash
node scripts/audit-artwork.cjs work/artwork.json
```

仅检查配置指定的单一正文目标内的可见图片，排除 Logo；使用源文件字节识别重复，即使文件改名或用 CSS 裁切也会被发现。未解释的重复使 CLI 返回退出码 1。原图透明统计与不透明边缘色差只作提示，不能代替对成品边界、纹理和光照的视觉检查。配置、允许重复的理由及报告范围见 [配图检查](artwork-audit.md)；近似重复、CSS 背景和其他非 `<img>` 画面仍需看全篇联系表。
